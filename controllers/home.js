...
router.get('/home/review', function(req, res){
	if(!req.session.cart){
		req.session.cart = {totalQty:0};
	   componentModel.getCart(function(result){
		   if(result.length > 0){
   ...
   componentModel.getPc(function(result){
	   res.render('home/review', {pcList: result, home: req.session.home });
   });
...
router.get('/home/pcset', function(req, res){
 	if(!req.session.cart){
 		req.session.cart = {totalQty:0};
		componentModel.getCart(function(result){
			if(result.length > 0){
	...
	componentModel.getPc(function(result){
		res.render('home/pcset', {pcList: result, home: req.session.home });
	});
